...
if __name__ == '__main__':
    ...
    utt_id2vad = {}
    for scp in sorted(glob(p_join(mfcc_scp_dir, 'vad_*.scp'))):
...
################# mfcc_scp안에 mfcc안에 있는 scp를 죄다 넣어야 한다
    for name in ['train', 'voxceleb1_test']:
        for i, mfcc in enumerate(glob(p_join('./mfcc', 'raw_mfcc_{}.*.scp'.format(name)))):
                ...
                with WriteHelper('ark,scp:{},{}'.format(vad_ark, vad_scp)) as writer:
                    for utt_id, high_resolution_mfcc in reader:
                        ...
                        try:
                            ...
                            high_resoultion_vad = np.zeros(high_resolution_mfcc.shape,
