...
nnet_dir="/media/sangjik/hdd2/speaker_verification/kaldi/xvector_nnet_1a.voxceleb1"
...
if [ $stage -le 1 ]; then
  ...
  for name in train voxceleb1_test; do
    steps/make_mfcc_zeek.sh --write-utt2num-frames true --mfcc-config conf/mfcc.conf --nj $num_cpu --cmd "$train_cmd" \
...
#  utils/subset_data_dir.sh data/train_aug 10000 data/train_aug_1m
#  utils/fix_data_dir.sh data/train_aug_1m
#
#  # Make MFCCs for the augmented data.  Note that we do not compute a new
#  # vad.scp file here.  Instead, we use the vad.scp from the clean version of
#  # the list.
#  steps/make_mfcc_zeek.sh --mfcc-config conf/mfcc.conf --nj $num_cpu --cmd "$train_cmd" \
...
local/nnet3/xvector/run_xvector.tmp.sh --stage $stage --train-stage -1 \
