...
voxceleb1_root=/media/sangjik/hdd2/dataset/speech/English/vox_1_subset/
voxceleb2_root=/media/sangjik/hdd2/dataset/speech/English/vox_2_subset/
nnet_dir="/media/sangjik/hdd2/speaker_verification/kaldi/xvector_nnet_1a.subset"
...
if [ $stage -le 1 ]; then
  ...
  for name in train voxceleb1_test; do
    steps/make_mfcc_zeek_for_substitution.sh --write-utt2num-frames true --mfcc-config conf/mfcc.conf --nj 8 --cmd "$train_cmd" \
...
if [ $stage -le 3 ]; then
  ...
  utils/subset_data_dir.sh data/train_aug 50000 data/train_aug_1m
  utils/fix_data_dir.sh data/train_aug_1m

  # Make MFCCs for the augmented data.  Note that we do not compute a new
  # vad.scp file here.  Instead, we use the vad.scp from the clean version of
  # the list.
  steps/make_mfcc_zeek_for_substitution.sh --mfcc-config conf/mfcc.conf --nj 4 --cmd "$train_cmd" \
