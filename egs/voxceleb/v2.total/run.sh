...
nnet_dir="/media/sangjik/hdd2/speaker_verification/kaldi/xvector_nnet_1a__________"
...
if [ $stage -le 1 ]; then
  ...
  for name in train voxceleb1_test; do
    steps/make_mfcc.sh --write-utt2num-frames true --mfcc-config conf/mfcc.conf --nj 12 --cmd "$train_cmd" \
